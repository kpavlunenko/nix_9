...
public class Calendar {
    ...
    public Calendar() {
        inputTemplate = "dd-mmm-yyyy 00:00";
        outputTemplate = "dd-mmm-yyyy 00:00";
        setComponentsOfDateFormat();
    }

    public Calendar(Calendar calendar) {
        inputTemplate = calendar.inputTemplate;
        outputTemplate = calendar.outputTemplate;
    }
    ...
    public void plusDay(long count) {
        ...
        if (newYear != year) {
            plusMilliseconds = plusMilliseconds + (newYear / 4 - year / 4) * 24 * 60 * 60 * 1000;
        }
            ...
            switch (subStringsPartOfDateTemplate[i]) {
                ...
                case "mmm":
                    if (subStringsPartOfDate[i] == "") {
                        ...
                        valueOfPartDate.put("month", getNumberOfMonthByName(subStringsPartOfDate[i]));
                    }
                    break;
                case "yyyy":
                    if (subStringsPartOfDate[i] == "") {
    ...
    private int getCountDaysInMonth(int numberOfMonth, int yearForCheck) {
    ...
    private int getNumberOfMonthByName(String nameOfMonth) {
        switch (nameOfMonth) {
            case "January":
                return 1;
            case "February":
    ...
    private String getMonthFromNumber(int numberOfMonth) {
        ...
        throw new RuntimeException("Number of month is not valid.");
    }
}
